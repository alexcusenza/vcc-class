...
void classModFuncRead03h::set_SizeWords(int size)
{
    ...
    if (size % 2 == 0)
        sModbusFunc.SizeWords = (size/2);      // even
    else
        sModbusFunc.SizeWords = (size/2 + 1);  // odd
}
...
//{
//    if (x%2 == 0)
//        sModbusFunc.SizeWords = (x/2);		// even
//    else
//        sModbusFunc.SizeWords = (x/2 + 1);	// odd
//}
...
classModFuncWrite0Fh::classModFuncWrite0Fh (char a, int b)
{
    sModbusFunc.TransID = 0;
    sModbusFunc.ProtoID = 0;
    sModbusFunc.Length = 0;		// 7 bytes: uid(1) + func(1) + addr(2) + bits(2) + bytes(1)
    sModbusFunc.UnitID = a;
...
{
    //set_MsgByteCnt(x);
    sModbusFunc.SizeBits = x * 8;
    sModbusFunc.SizeBytes = x;
